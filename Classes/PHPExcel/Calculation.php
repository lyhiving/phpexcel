...
class PHPExcel_Calculation
{
    ...
    public static function unwrapResult($value)
    {
        if (is_string($value)) {
            ...
            if ((isset($values[0])) && ($values[0] == '"') && (substr($value, -1) == '"')) {
    ...
    public function parseFormula($formula)
    {
        ...
        $formulas = str_split($formula);
        if ((!isset($formulas[0])) || ($formulas[0] != '=')) {
            return array();
        }
    ...
    public function _calculateFormulaValue($formula, $cellID = null, PHPExcel_Cell $pCell = null)
    {
        ...
        $wsTitles = str_split($wsTitle);
        if (($wsTitles[0] !== "\x00") && ($this->cyclicReferenceStack->onStack($wsCellReference))) {
...
//echo 'Assessing Expression '.substr($formula, $index), PHP_EOL;
            $opCharacter = $formulas[$index];    //    Get the first character of the value at the current index position
//echo 'Initial character of expression block is '.$opCharacter, PHP_EOL;
            if ((isset(self::$comparisonOperators[$opCharacter])) && (strlen($formula) > $index) && (isset(self::$comparisonOperators[$formulas[$index+1]]))) {
                $opCharacter .= $formulas[++$index];
...
//                    echo 'Token is a PHPExcel constant: '.$excelConstant.'<br />';
                ...
                } elseif ((is_numeric($token)) || ($token === null) || (is_bool($token)) || ($token == '') || ($tokens[0] == '"') || ($tokens[0] == '#')) {
...
//        }
    ...
    private function executeBinaryComparisonOperation($cellID, $operand1, $operand2, $operation, &$stack, $recursingArrays = false)
    {
        ...
        $operand1s = str_split($operand1);
        $operand2s = str_split($operand2);
        //    Simple validate the two operands if they are string values
        if (is_string($operand1) && $operand1 > '' && $operand1s[0] == '"') {
            $operand1 = self::unwrapResult($operand1);
        }
        if (is_string($operand2) && $operand2 > '' && $operand2s[0] == '"') {
