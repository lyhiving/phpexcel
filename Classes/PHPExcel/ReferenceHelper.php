...
class PHPExcel_ReferenceHelper
{
    ...
    private function updateSingleCellReference($pCellReference = 'A1', $pBefore = 'A1', $pNumCols = 0, $pNumRows = 0)
    {
        if (strpos($pCellReference, ':') === false && strpos($pCellReference, ',') === false) {
            ...
            $newColumns = str_split($newColumn);
            $beforeColumns = str_split($beforeColumn);
            $beforeRows = str_split($beforeRow);
            $newRows = str_split($newRow);
            $updateColumn = (($newColumns[0] != '$') && ($beforeColumns[0] != '$') && (PHPExcel_Cell::columnIndexFromString($newColumn) >= PHPExcel_Cell::columnIndexFromString($beforeColumn)));
            $updateRow = (($newRows[0] != '$') && ($beforeRows[0] != '$') && $newRow >= $beforeRow);
