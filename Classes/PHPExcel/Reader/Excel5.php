...
//        echo '<br />';
    ...
    private function readDateMode()
    {
        ...
        $recordDatas = str_split($recordData);
        if (ord($recordDatas[0]) == 1) {
    ...
    private function readFont()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $recordDatas = str_split($recordData);
            $underlineType = ord($recordDatas[10]);
    ...
    private function readXf()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $recordDatas = str_split($recordData);
            $horAlign = (0x07 & ord($recordDatas[6])) >> 0;
            switch ($horAlign) {
                case 0:
            ...
            $wrapText = (0x08 & ord($recordDatas[6])) >> 3;
            switch ($wrapText) {
                case 0:
            ...
            $vertAlign = (0x70 & ord($recordDatas[6])) >> 4;
            switch ($vertAlign) {
                case 0:
            ...
            if ($this->version == self::XLS_BIFF8) {
                ...
                $indent = (0x0F & ord($recordDatas[8])) >> 0;
                $objStyle->getAlignment()->setIndent($indent);

                // bit: 4; mask: 0x10; 1 = shrink content to fit into cell
                $shrinkToFit = (0x10 & ord($recordDatas[8])) >> 4;
                switch ($shrinkToFit) {
                    case 0:
                ...
                $recordDatas = str_split($recordData);
                $orientationAndFlags = ord($recordDatas[7]);
            ...
            while ($offset < $length) {
                ...
                switch ($extType) {
                    case 4:        // fill start color
                        ...
                        $xclrValues = str_split($xclrValue);
                        if ($xclfType == 2) {
                            $rgb = sprintf('%02X%02X%02X', ord($xclrValues[0]), ord($xclrValues[1]), ord($xclrValues[2]));
    ...
    private function readFormula()
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            $recordDatas = str_split($recordData);
            if ((ord($recordDatas[6]) == 0) && (ord($recordDatas[12]) == 255) && (ord($recordDatas[13]) == 255)) {
            ...
            } elseif ((ord($recordDatas[6]) == 1)
                && (ord($recordDatas[12]) == 255)
                && (ord($recordDatas[13]) == 255)) {
                // Boolean formula. Result is in +2; 0=false, 1=true
                $dataType = PHPExcel_Cell_DataType::TYPE_BOOL;
                $value = (bool) ord($recordDatas[8]);
            } elseif ((ord($recordDatas[6]) == 2)
                && (ord($recordDatas[12]) == 255)
                && (ord($recordDatas[13]) == 255)) {
                // Error formula. Error code is in +2
                $dataType = PHPExcel_Cell_DataType::TYPE_ERROR;
                $value = PHPExcel_Reader_Excel5_ErrorCode::lookup(ord($recordDatas[8]));
            } elseif ((ord($recordDatas[6]) == 3)
                && (ord($recordDatas[12]) == 255)
                && (ord($recordDatas[13]) == 255)) {
    ...
    private function readBoolErr()
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            $recordDatas = str_split($recordData);
            $boolErr = ord($recordDatas[6]);

            // offset: 7; size: 1; 0=boolean; 1=error
            $isError = ord($recordDatas[7]);
